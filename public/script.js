...
import { firebaseConfig } from "./config.min.js";
import { sendEmailWithQRCode } from "./transmit.min.js";
    ...
    onValue(teachersRef, async (snapshot) => {
        ...
        for (const teacherID in data) {
            try {
                ...
                const hasActiveEvents = await fetchAndCheckEvents(teacherID);
                if (hasActiveEvents) {
        ...
        await hideTeacherNamesWithoutActiveEvents();
    });
}
        ...
        onValue(eventsRef, (snapshot) => {
            ...
            if (events) {
                for (const eventName in events) {
                    ...
                    const startDateTime = parseCustomDateFormat(eventData.StartDateTime);
                    const endDateTime = parseCustomDateFormat(eventData.EndDateTime);
...
document.getElementById('teacherOptions').addEventListener('change', async () => {
    ...
    // Create and prepend placeholder option "Select Event"
    const placeholderOption = document.createElement('option');
    placeholderOption.value = '';
    placeholderOption.textContent = 'Select Event';
    placeholderOption.disabled = true
    placeholderOption.selected = true
    eventOptions.appendChild(placeholderOption);

    // Fetch and display events for the selected teacher
    const currentDateTime = new Date();
    const eventsRef = ref(db, "Teacher's Form/" + selectedTeacher + "/Events");
    onValue(eventsRef, (snapshot) => {
        const events = snapshot.val();
    ...
    // Hide teacher names without active events
    await hideTeacherNamesWithoutActiveEvents();
});


function calculateDaysAndTimeLeft(startDateTime, endDateTime) {
    const startDate = parseCustomDateFormat(startDateTime);
    const endDate = parseCustomDateFormat(endDateTime);
...
async function generateQRCode() {
    ...
    if (studentData.length > 0) {
        ...
        let qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodedData}`;
